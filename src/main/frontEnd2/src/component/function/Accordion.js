...
const Del = (e) => {
    ...
    axios.delete(`/calendar/${parent.parentElement.id}`)
...
const EditConfirm = (e) => {
    ...
    parent.parentElement.querySelectorAll('input')
        ...
        const param = {
            ...
            //수정
            axios.patch(`/calendar/${div.id}`, param)
            .then((res) => {
                alert('수정되었습니다.');
                parent.classList.remove('slide');
                parent.previousElementSibling.classList.remove('slide');
        ...
        <div>
            ...
            <Content>
                <section className='list'>
                    {searchResults.map(item => <Question key={item.exerciseLogId} id={item.exerciseLogId} tit={item.content} subTit={item.detailLog} count={item.number} />)}
                </section>   
            </Content>
        </div>
    )
