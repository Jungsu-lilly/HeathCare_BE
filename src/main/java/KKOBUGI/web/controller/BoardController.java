...
public class BoardController {
     ...
     * */
    @GetMapping("/board")
    public List<ResDetailDto> findAll(){
        List<Board> boards = boardService.findAll();
        List<ResDetailDto> boardDtos = new ArrayList<>();
        for (Board b : boards) {
            boardDtos.add(new ResDetailDto(b.getId(),b.getTitle(),b.getContent(),b.getCreateDate(),b.getModifyDate(),b.getUser().getNickname()));
        }
        return boardDtos;
    }
    ...
    public BoardDto.BoardDetailDto boardDetail(@PathVariable("boardId") Long id) {
        ...
        List<CommentDtos> commentDtos = board.getComments().stream().map(o -> new CommentDtos(o.getId(),o.getUser().getNickname(), o.getContent())).collect(toList());
